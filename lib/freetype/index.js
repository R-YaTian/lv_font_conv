...
module.exports.glyph_render = function (face, code, opts = {}) {
  if (opts.auto_shrink)
  {
    // reinit font size at this time
    FT_Set_Char_Size(face.ptr, 0, opts.pixel_size * 64, 300, 300);
    FT_Set_Pixel_Sizes(face.ptr, 0, opts.pixel_size);
  }
  ...
  {
    let i = 0;
    while (from_26_6(m.getValue(glyph + m.OFFSET_GLYPH_METRICS_HEIGHT, 'i32')) > opts.pixel_size)
    {
      i += 1;
      FT_Set_Char_Size(face.ptr, 0, (opts.pixel_size - i) * 64, 300, 300);
      FT_Set_Pixel_Sizes(face.ptr, 0, opts.pixel_size - i);
      FT_Load_Glyph(face.ptr, glyph_index, load_flags);
      glyph = m.getValue(face.ptr + m.OFFSET_FACE_GLYPH, 'i32');
    }
  }
